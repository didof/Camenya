...
struct TakeThumbnailGenerator {
    typealias ThumbnailDataProvider = @Sendable (URL) async throws -> Data

    private let thumbnailData: ThumbnailDataProvider

    init(thumbnailData: ThumbnailDataProvider? = nil) {
        self.thumbnailData = thumbnailData ?? { movieURL in
            try await TakeThumbnailGenerator.renderThumbnail(movieURL: movieURL)
        }
    }

    func generate(movieAt movieURL: URL, destination: URL) async throws {
        let data = try await thumbnailData(movieURL)
    ...
    private static func renderThumbnail(movieURL: URL) async throws -> Data {
        let asset = AVURLAsset(url: movieURL)
        let duration = try await asset.load(.duration)
        let seconds = max(0, min(0.35, duration.seconds * 0.1))
